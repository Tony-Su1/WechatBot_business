...
# 私聊客户经营档案
# SQLite 是客户资料真实来源，CustomerMemory/ 保存给模型看的摘要缓存。
...
# 是否使用 OpenAI Responses API 的内置 web_search 工具（真正联网检索）
ONLINE_USE_WEB_SEARCH_TOOL = True
# 返回的参考来源数量（仅用于调试/追踪，可在最终回复阶段隐藏）
ONLINE_WEB_MAX_SOURCES = 5
...
# 触发后持续监听窗口（秒）
GROUP_FOLLOWUP_WINDOW_SECONDS = 30

# session 内是否强制处理（持续监听）
GROUP_SESSION_FORCE_PROCESS = True

# 触发点（@/关键词）是否绕过概率，稳定进入对话
GROUP_SESSION_BYPASS_PROBABILITY = True

# session 内每条消息是否滚动刷新窗口（只要群里在聊就不断线）
GROUP_SESSION_REFRESH_ON_ANY_MESSAGE = True

# 给模型的群聊上下文条数
GROUP_CONTEXT_MAX_MESSAGES = 8

# 会话窗口内：任意新消息都刷新窗口（建议 True）
GROUP_SESSION_REFRESH_ON_ANY_MESSAGE = True

# 机器人在群里发回复时也刷新窗口（建议 True）
GROUP_SESSION_REFRESH_ON_BOT_REPLY = True
