...
# - Older chat models continue to accept `max_tokens`.
# This wrapper keeps the rest of the codebase unchanged.
# ----------------------------------------------------------------------
def _model_uses_max_completion_tokens(model_name: str) -> bool:
    try:
        ...
        name = ""
    # GPT-5 family (gpt-5, gpt-5.1, gpt-5.1-instant, gpt-5.2, etc.)
    return name.startswith("gpt-5")

def _chat_completions_create(client_obj, *, model: str, messages, temperature: float, max_tokens: int = None,
...
ADMIN_CMD_PREFIX = "/admin"
HOT_SECTION_TITLE = "## 热更新叠加区域"

def _get_admin_nicknames():
    # 优先动态读取，避免修改 config.py 后必须重启
    admins = get_dynamic_config('BOT_ADMIN_NICKNAMES', None)
    if admins is None:
        # 兼容：config.py 可能没有该字段
        try:
            admins = BOT_ADMIN_NICKNAMES
        except Exception:
            admins = []
    # 规范成 list[str]
    ...
    if not isinstance(admins, (list, tuple)):
        admins = []
    return [str(x) for x in admins if x]

def _is_admin_sender(sender_name: str) -> bool:
    try:
        admins = _get_admin_nicknames()
        return str(sender_name) in admins
    except Exception:
        return False


def _admin_send_message(who: str, text: str):
    """管理员指令的即时回复：不入队列，直接发微信消息。"""
    try:
        if not text:
            return
        # 参考 send_reply 的发送逻辑，但更轻量（不模拟打字，不做分段）
        for attempt in range(3):
            try:
                time.sleep(0.1)
                logger.info(f"[ADMIN] 发送给 {who}: {repr(text[:200])}")
                _wx_send_msg(who, text)
                return
            except Exception as e:
                if attempt == 2:
                    logger.error(f"[ADMIN] SendMsg失败（已重试3次）: {e}", exc_info=True)
                else:
                    time.sleep(0.3)
    except Exception as e:
        logger.error(f"[ADMIN] _admin_send_message异常: {e}", exc_info=True)
def _get_prompt_path_for_user(user_id: str) -> str:
    prompt_file = prompt_mapping.get(user_id, user_id)
    return os.path.join(root_dir, 'prompts', f'{prompt_file}.md')

def _ensure_hot_section_exists(prompt_content: str) -> str:
    if HOT_SECTION_TITLE in prompt_content:
        return prompt_content
    # 如果 prompt 有“记忆片段”，把热更新区插在记忆片段之前，避免被“移除记忆片段逻辑”裁掉
    memory_marker = "## 记忆片段"
    hot_block = (
        "\n\n" + HOT_SECTION_TITLE + "\n"
        "（此区域由 /admin hot 动态追加，重启后仍保留；越往下优先级越高）\n"
    )
    if memory_marker in prompt_content:
        head, tail = prompt_content.split(memory_marker, 1)
        head = head.rstrip()
        return head + hot_block + "\n\n" + memory_marker + tail
    return prompt_content.rstrip() + hot_block

def _append_hot_rule(user_id: str, rule_text: str) -> str:
    prompt_path = _get_prompt_path_for_user(user_id)
    if not os.path.exists(prompt_path):
        return f"❌ Prompt 文件不存在：{prompt_path}"
    raw = safe_read_file_with_encoding(prompt_path, fallback_content="")
    raw = _ensure_hot_section_exists(raw)
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    block = f"\n- [{ts}] {rule_text.strip()}\n"
    # 追加到热更新区末尾：简单做法是直接在文件末尾追加；如果热更新区不在末尾，也能工作（规则仍在热更新区之后）
    # 为保证追加发生在热更新区内：我们定位热更新区标题，追加到其后（直到下一个二级标题或文末）
    try:
        ...
        while i < len(lines):
            out.append(lines[i])
            if not inserted and lines[i].strip() == HOT_SECTION_TITLE:
                # 跳过标题行后，继续收集，直到遇到下一个 '## ' 标题（且不是热更新标题本身）
                i += 1
                # 先把紧随其后的说明行也写入
                while i < len(lines) and (lines[i].strip() == "" or not lines[i].lstrip().startswith("## ")):
                    out.append(lines[i])
                    i += 1
                # 这里插入新规则（在下一个 section 前）
                out.append(block)
                inserted = True
                continue
            i += 1
        ...
        new_content = raw + "\n" + block
    safe_write_file_with_encoding(prompt_path, new_content, mode='w')
    return "✅ 已追加热更新规则。"

def _show_hot_rules(user_id: str) -> str:
    prompt_path = _get_prompt_path_for_user(user_id)
    if not os.path.exists(prompt_path):
        return f"❌ Prompt 文件不存在：{prompt_path}"
    raw = safe_read_file_with_encoding(prompt_path, fallback_content="")
    if HOT_SECTION_TITLE not in raw:
        return "（当前没有热更新叠加区域）"
    # 抽取热更新区块：从标题到下一个 '## ' 或文末
    lines = raw.splitlines()
    start = None
    for i, ln in enumerate(lines):
        if ln.strip() == HOT_SECTION_TITLE:
            start = i
            break
    if start is None:
        return "（当前没有热更新叠加区域）"
    ...
    for j in range(start + 1, len(lines)):
        if lines[j].lstrip().startswith("## ") and lines[j].strip() != HOT_SECTION_TITLE:
            end = j
            break
    snippet = "\n".join(lines[start:end]).strip()
    return snippet if snippet else "（热更新叠加区域为空）"

def _clear_hot_rules(user_id: str) -> str:
    prompt_path = _get_prompt_path_for_user(user_id)
    if not os.path.exists(prompt_path):
        return f"❌ Prompt 文件不存在：{prompt_path}"
    raw = safe_read_file_with_encoding(prompt_path, fallback_content="")
    ...
    while i < len(lines):
        ln = lines[i]
        if ln.strip() == HOT_SECTION_TITLE and not cleared:
            out.append(ln)
            i += 1
            # 保留标题后紧跟的说明/空行，直到遇到下一个 '## '（二级标题）或文末
            while i < len(lines) and (lines[i].strip() == "" or not lines[i].lstrip().startswith("## ")):
                # 仅保留第一行说明（若有），其余清空
                if "（此区域由 /admin hot" in lines[i]:
                    out.append(lines[i])
                i += 1
            # 写一个换行，表示清空完毕
            out.append("\n")
        ...
        out.append(ln)
        i += 1
    safe_write_file_with_encoding(prompt_path, "".join(out), mode='w')
    return "✅ 已清空热更新叠加区域。"

def _handle_admin_command_if_any(msg, who: str, sender: str, is_group_chat: bool) -> bool:
    """优先处理 /admin ... 指令。返回 True 表示已处理并终止后续逻辑。"""
    try:
        if msg is None:
            return False
        raw = (msg.content or "").strip()
        if not raw.startswith(ADMIN_CMD_PREFIX):
            return False
        # 仅允许管理员执行
        if not _is_admin_sender(sender):
            _admin_send_message(who, "❌ 无权限执行管理员指令。请在 config.py 配置 BOT_ADMIN_NICKNAMES = ['你的昵称']。")
            return True

        # 解析：/admin hot <text> | /admin hot_show | /admin hot_clear
        parts = raw.split(None, 2)
        if len(parts) < 2:
            _admin_send_message(who, "用法：/admin hot <规则> | /admin hot_show | /admin hot_clear")
            return True
        sub = parts[1].strip().lower()

        # 目标 prompt 默认使用聊天窗口 who（群聊=群名，私聊=对话名）
        target_user_id = who

        if sub == "hot":
            if len(parts) < 3 or not parts[2].strip():
                _admin_send_message(who, "用法：/admin hot <要追加的规则文本>")
                return True
            rule = parts[2]
            # 防止把多行折叠：允许携带 \n
            rule = rule.replace("\\n", "\n")
            res = _append_hot_rule(target_user_id, rule)
            _admin_send_message(who, res)
            return True
        elif sub in ("hot_show", "hotshow"):
            snippet = _show_hot_rules(target_user_id)
            _admin_send_message(who, snippet)
            return True
        elif sub in ("hot_clear", "hotclear"):
            res = _clear_hot_rules(target_user_id)
            _admin_send_message(who, res)
            return True
        else:
            _admin_send_message(who, "未知子命令。用法：/admin hot <规则> | /admin hot_show | /admin hot_clear")
            return True
    except Exception as e:
        try:
            logger.error(f"处理 /admin 指令失败: {e}", exc_info=True)
        except Exception:
            pass
        return False
...
recent_bot_sent_texts = {}
recent_bot_sent_texts_lock = threading.Lock()
...
# wxautox4 通过 Windows UI 操作微信，所有窗口操作必须串行执行。
wx_operation_lock = threading.RLock()


def _wx_send_msg(who: str, text: str):
    with wx_operation_lock:
        return wx.SendMsg(msg=text, who=who)


def _wx_send_file(who: str, filepath: str):
    with wx_operation_lock:
        return wx.SendFiles(filepath=filepath, who=who)


def _wx_add_listener(nickname: str):
    with wx_operation_lock:
        return wx.AddListenChat(nickname=nickname, callback=message_listener)


def _record_bot_sent_text(who: str, text: str) -> None:
    normalized = str(text or "").strip()
    if not normalized:
        return
    now = time.time()
    with recent_bot_sent_texts_lock:
        entries = recent_bot_sent_texts.setdefault(who, deque(maxlen=30))
        entries.append((normalized, now))


def _is_recent_bot_sent_text(who: str, text: str, max_age_seconds: float = 30.0) -> bool:
    normalized = str(text or "").strip()
    if not normalized:
        return False
    now = time.time()
    with recent_bot_sent_texts_lock:
        entries = recent_bot_sent_texts.get(who)
        if not entries:
            return False
        while entries and now - entries[0][1] > max_age_seconds:
            entries.popleft()
        for index, (sent_text, _) in enumerate(entries):
            if sent_text == normalized:
                del entries[index]
                return True
    return False
...
def message_listener(msg, chat):
    ...
    if msgtype == 'text' and _is_recent_bot_sent_text(who, original_content):
        logger.info(f"检测到机器人近期已发送的回显文本，已忽略。聊天: {who}, 内容: {original_content[:100]}")
        return

    # 某些群聊场景中，wxautox4 可能把机器人自己发送的消息标记为 friend。
    # 同时按发送者昵称过滤，防止机器人把自己的回复再次送入 AI 队列。
    normalized_sender = str(sender or "").strip()
    if normalized_sender == str(ROBOT_WX_NAME).strip() or normalized_sender.lower() in ("self", "me"):
        global bot_last_sent_msg
        bot_last_sent_msg[who] = msg
        logger.info(f"检测到机器人自身消息，已忽略。聊天: {who}, 发送者: {sender}, 属性: {msgattr}")
        return
        ...
        try:
            ...
            for username in inactive_users:
                try:
                    ...
                    # 单个用户处理失败不能让整个普通回复线程永久退出。
                    logger.error(f"处理用户 {username} 的消息失败，普通回复线程将继续运行: {exc}", exc_info=True)
        except Exception as exc:
            logger.error(f"普通回复检查线程发生异常，将继续运行: {exc}", exc_info=True)
    ...
    try:
        ...
        for idx, (action_type, content) in enumerate(message_actions):
            if action_type == 'emoji':
                ...
                for attempt in range(3):
                    try:
                        if _wx_send_file(user_id, content):
                ...
                for attempt in range(3):
                    try:
                        ...
                        send_result = _wx_send_msg(user_id, content)
                        logger.info(f"[DEBUG] SendMsg返回结果: {send_result}, 内容长度: {len(content)}")
                        if send_result:
                            _record_bot_sent_text(user_id, content)
    ...
    try:
        ...
        for user_name in user_names:
            if user_name == ROBOT_WX_NAME:
                ...
                exit(1)
            ListenChat = _wx_add_listener(user_name)
            if ListenChat:
